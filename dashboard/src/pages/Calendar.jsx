    ...
    <div className="p-2 md:p-4 max-w-6xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row w-full md:w-auto gap-2">
          ...
          <div className="flex gap-2">
            <input
              ...
              placeholder="Filter..."
              className="border p-1 md:p-2 rounded w-full md:w-48 text-sm md:text-base"
            />
            {events.length > 0 && (
              <button
      ...
      <div className="grid grid-cols-7 gap-1 text-xs md:text-sm">
        ...
        {['S', 'M', 'T', 'W', 'T', 'F', 'S'].map((day, i) => (
          <div key={i} className="text-center font-semibold py-1 md:py-2">
            {isMobile ? day : ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'][i]}
          </div>
        ))}
        ...
        {daysInMonth.map((day, i) => {
          ...
          return (
            <div
              key={i}
              className={`h-fit md:min-h-24 p-1 md:p-2 border rounded-lg ${
      ...
      {/* Event List - Hidden on mobile when filtering */}
